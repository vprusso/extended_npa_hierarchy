...
\begin{tikzpicture}[text depth=1pt, bend angle=45]
	...
	\node[Alice] (Alice-1-n) at (0,3.3) {$\rho_n$};

	% P_{a_1} ... P_{a_n}
	\node[Alice] (Alice-2-1) at (6,0) {\small $\{P_{a_1}\}$};
	\node[Alice] (Alice-2-2) at (6,1.5) {\small $\{P_{a_2} \}$};	
	\node[] (elips-2) at (6,2.5) {$\vdots$};
	\node[Alice] (Alice-2-n) at (6,3.3) {\small $\{P_{a_n} \}$};	
	
	% Bob (Phi)
    \node[Bob] (Bob) at (3,5.5) {\large $\Phi_n$};
    ...
    \draw[->] (Alice-1-n) -- node[above]{\small $\reg{Z}_n$} (Alice-2-n);
   	...
   	\draw[->] (Alice-1-n) edge[->, bend left=30] node[above left]{\small $\reg{X}_n$} (Bob); 
   	...
   	\draw[->] (Bob) edge[->, bend left=30] node[above right]{\small $\reg{Y}_n$} (Alice-2-n); 
\end{tikzpicture}
\end{center}
\end{document}
