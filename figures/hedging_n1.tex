...
\begin{tikzpicture}[text depth=1pt, bend angle=45]
	...
	% rho_1 ... rho_n
	\node[Alice] (Alice-1-1) at (0,0) {$\rho$};

	% P_{a_1} ... P_{a_n}
	\node[Alice] (Alice-2-1) at (6,0) {\small $\{P_{a}\}$};

	% Bob (Phi)
    \node[Bob] (Bob) at (3,5.5) {\large $\Phi$};
    
    \draw[->] (Alice-1-1) -- node[above]{\small $\reg{Z}$} (Alice-2-1);

   	\draw[->] (Alice-1-1) edge[->, bend left=0] node[above left]{\small $\reg{X}$} (Bob); 

   	\draw[->] (Bob) edge[->, bend left=0] node[above right]{\small $\reg{Y}$} (Alice-2-1); 
\end{tikzpicture}
\end{center}
\end{document}
